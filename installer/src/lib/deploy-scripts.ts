...
export type DeploymentStepId = 'connect' | 'prepare' | 'upload' | 'env' | 'deps' | 'build' | 'nginx' | 'ssl' | 'cloudflare' | 'start';
...
export function generateNginxConfig(config: DeploymentConfig): string {
  ...
  const port = 3000;
  const useCloudflare = config.cloudflare?.enabled ?? false;

  // When behind Cloudflare, we need to trust their proxy IPs
  // so Nginx can extract the real visitor IP from CF-Connecting-IP header.
  const cloudflareRealIpBlock = useCloudflare
    ? `
    # ── Cloudflare: Restore real visitor IP ──
    # Source: https://www.cloudflare.com/ips/
${CLOUDFLARE_IPV4_RANGES.map(ip => `    set_real_ip_from ${ip};`).join('\n')}
${CLOUDFLARE_IPV6_RANGES.map(ip => `    set_real_ip_from ${ip};`).join('\n')}
...
# Generated by Installer${useCloudflare ? ' (Cloudflare-optimized)' : ''}
...
export async function deployToServer(
  ...
  <text x="10" y="35" font-family="Arial, sans-serif" font-size="24" font-weight="bold" fill="${config.domain.primaryColor}">
    ...
    // ============================================
    // Step 9 (conditional): Cloudflare Setup
    // ============================================
    if (config.cloudflare?.enabled && config.cloudflare.apiToken && config.cloudflare.accountId) {
      currentStep = 'cloudflare';
      onProgress({ stepId: 'cloudflare', status: 'running', message: 'Setting up Cloudflare...' });

      try {
        const cfResult = await setupCloudflare(
          config.cloudflare.apiToken,
          config.cloudflare.accountId,
          domain,
          config.server.host,
          (msg) => onProgress({ stepId: 'cloudflare', status: 'running', log: msg })
        );

        onProgress({
          stepId: 'cloudflare',
          status: 'completed',
          message: 'Cloudflare configured',
          log: `\n⚠️  ACTION REQUIRED: Update your domain nameservers at your registrar to:\n    ${cfResult.nameservers.join('\n    ')}\n\nDNS propagation can take up to 48 hours.\n`,
        });
      } catch (cfError) {
        // Cloudflare setup failure is non-fatal — the site works without it
        const cfMsg = cfError instanceof Error ? cfError.message : 'Cloudflare setup failed';
        onProgress({
          stepId: 'cloudflare',
          status: 'completed',
          message: `Cloudflare warning: ${cfMsg}`,
          log: `⚠️ Cloudflare setup encountered an issue: ${cfMsg}. Your site will still work without Cloudflare. You can set it up manually later.`,
        });
      }
    }
    ...
    // Deployment Complete
    // ============================================
    const cloudflareNote = config.cloudflare?.enabled
      ? `
Cloudflare: ENABLED
  ⚠️  Update your nameservers at your domain registrar!
  Your site won't be proxied through Cloudflare until
  nameservers are updated and DNS propagates (up to 48h).
`
      : '';
